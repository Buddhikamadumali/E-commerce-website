...
function ProductCard({id, image, name, description, price}) {
  return (
    <div className='bg-gray-700 text-white rounded-lg overflow-hidden shadow-lg max-w-xs transition-shadow duration-300'>
      <Link to={`/product/${id}`}>
      
      <img src={image} alt={name} className='w-full max-h-60 object-contain bg-white '/>
      <div className='p-4 flex flex-col'>
        <h2 className='text-lg text-center font-bold mb-1 text-orange-500'>{name}</h2>
